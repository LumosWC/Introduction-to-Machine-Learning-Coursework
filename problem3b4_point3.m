...
for i = 1:trainingdata
    ...
    if(X(3,i)-X(2,i)<0.1+0.3)&&(X(3,i)-X(2,i)>0.1)
        Y(i)=0;
        X(:,i)=zeros(1,3)';
    end    
...
for i = 1:1000
    if X_test(3,i)-X_test(2,i) >= 0.1+0.3
        Y_test(i) = 1;
    ...
    if X_test(3,i)-X_test(2,i) < 0.1
     ...
     if (X_test(3,i)-X_test(2,i) < 0.1+0.3)&& (X_test(3,i)-X_test(2,i) > 0.1)
        Y_test(i)=0;
        X_test(:,i) = zeros(1,3)';
    end
end
...
ylabel('Test Error (%)');
title('3b4-Test Error against Number of Training Points-Sep=0.3')